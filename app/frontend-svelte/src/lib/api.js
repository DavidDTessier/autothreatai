...
export async function uploadFile(file, signal) {
  ...
  const data = await response.json();
  if (data.status !== 'success') throw new Error(data.error ?? 'File upload failed');
  return {
    data: data.data,
...
export async function deleteUploadedFile(filename) {
  if (!filename) return;
  const response = await fetch(`${API_BASE}/api/upload/${encodeURIComponent(filename)}`, {
    method: 'DELETE',
    headers: { 'Content-Type': 'application/json' },
  });
  if (!response.ok) {
...
export async function streamQuery(
  { user_id, session_id, message_parts, api_key, use_vertex, vertex_project, vertex_location, model_id },
  { signal, onChunk }
) {
  const body = {
    user_id,
  ...
  if (!response.ok) {
    ...
    try {
      ...
      if (j.detail) msg = typeof j.detail === 'string' ? j.detail : msg + ' - ' + text;
    } catch (_) {
      if (text) msg += ' - ' + text;
    }
    throw new Error(msg);
